...
interface IXswapFarm {
    ...
    // Harvest rewards tokens from the farm.
    function harvest(uint256 pid, address to) external;

    // Withdraw LP and rewards tokens from the farm.
...
contract STRATAPE is ERC20, Ownable, ReentrancyGuard, Pausable {
    // Maximises yields in e.g. apeswap.
    ...
    address public constant UNI_ROUTER_ADDRESS =
        0xC0788A3aD43d79aa53B09c2EaCc313A787d1d607; // uniswap, pancakeswap etc
    ...
    address public constant WMATIC_ADDRESS =
        0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; // should be WMATIC or USDC
    ...
    constructor(
        ...
        if (IS_AUTO_COMP) {
            ...
            if (WMATIC_ADDRESS == earnedAddress) {
                earnedToNATIVEPath = [WMATIC_ADDRESS, retroAddress];
            }

            earnedToToken0Path = [earnedAddress, WMATIC_ADDRESS, token0Address];
            if (WMATIC_ADDRESS == token0Address) {
                earnedToToken0Path = [earnedAddress, WMATIC_ADDRESS];
            }

            earnedToToken1Path = [earnedAddress, WMATIC_ADDRESS, token1Address];
            if (WMATIC_ADDRESS == token1Address) {
                earnedToToken1Path = [earnedAddress, WMATIC_ADDRESS];
            }

            token0ToEarnedPath = [token0Address, WMATIC_ADDRESS, earnedAddress];
            if (WMATIC_ADDRESS == token0Address) {
                token0ToEarnedPath = [WMATIC_ADDRESS, earnedAddress];
            }

            token1ToEarnedPath = [token1Address, WMATIC_ADDRESS, earnedAddress];
            if (WMATIC_ADDRESS == token1Address) {
                token1ToEarnedPath = [WMATIC_ADDRESS, earnedAddress];
            }

            earnedToWantPath = [earnedAddress, WMATIC_ADDRESS, wantAddress];
            if (WMATIC_ADDRESS == wantAddress) {
                earnedToWantPath = [earnedAddress, wantAddress];
            }

            earnedToWMATICPath = [earnedAddress, WMATIC_ADDRESS];
            wmaticToNativePath = [WMATIC_ADDRESS, retroAddress];
        }
    ...
    function _farm() internal {
        ...
        IXswapFarm(FARM_CONTRACT_ADDRESS).deposit(pid, wantAmt, address(this));
    }
        ...
        returns (uint256)
    {
        ...
        if (b < r) {
            ...
            if (IS_AUTO_COMP) {
                IXswapFarm(FARM_CONTRACT_ADDRESS).withdrawAndHarvest(
                    pid,
                    _withdraw,
                    address(this)
                );
    ...
    function earn() external nonReentrant whenNotPaused {
        ...
        // Harvest farm tokens
        IXswapFarm(FARM_CONTRACT_ADDRESS).harvest(pid, address(this));
    ...
    function buyBack(uint256 earnedAmt) internal returns (uint256) {
        ...
        if (UNI_ROUTER_ADDRESS != buybackRouterAddress) {
            ...
            if (earnedAddress != WMATIC_ADDRESS) {
            ...
            uint256 wmaticAmt = IERC20(WMATIC_ADDRESS).balanceOf(address(this));
            if (wmaticAmt > 0) {
                IERC20(WMATIC_ADDRESS).safeIncreaseAllowance(
    ...
    function pause() external onlyAllowGov {
        _pause();
        IXswapFarm(FARM_CONTRACT_ADDRESS).emergencyWithdraw(pid, address(this));
    }
    ...
    function changeRetroAddress(address newRetroAddress) external onlyAllowGov {
        ...
        earnedToNATIVEPath = [earnedAddress, WMATIC_ADDRESS, newRetroAddress];
        if (WMATIC_ADDRESS == earnedAddress) {
            earnedToNATIVEPath = [WMATIC_ADDRESS, newRetroAddress];
        }
        wmaticToNativePath = [WMATIC_ADDRESS, newRetroAddress];
    }

    function workerCompound() external view returns (uint256) {
        uint256 BalanceTokens = IERC20(earnedAddress).balanceOf(address(this));
        uint256 Comp = IXswapFarm(FARM_CONTRACT_ADDRESS).pendingBanana(
            pid,
            address(this)
        );

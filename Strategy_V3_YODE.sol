...
contract STRATYODE is ERC20, Ownable, ReentrancyGuard, Pausable {
    // Maximises yields in e.g. YODESwap.
    ...
    address public constant UNI_ROUTER_ADDRESS =
        0x72d85Ab47fBfc5E7E04a8bcfCa1601D8f8cE1a50; // uniswap, pancakeswap etc
    ...
    address public constant WWDOGE_ADDRESS =
        0xB7ddC6414bf4F5515b52D8BdD69973Ae205ff101; // should be WWDOGE or USDC
    ...
    /* This is vanity address -  For instance an address 0x000000000000000000000000000000000000dEaD for which it's
       absolutely impossible to generate a private key with today's computers. SPECIAL CASE , NO TOKEN TEAM FEE FOR NEW POOLS IN SYNTH */
    //slither-disable-next-line too-many-digits
    address public BUY_BACK_ADDRESS =
    ...
    address[] public earnedToWWDOGEPath;
    address[] public wwdogeToNativePath;
    ...
    constructor(
        ...
        if (IS_AUTO_COMP) {
            ...
            FARM_CONTRACT_ADDRESS = 0xf7b1150cb31488bde3eB3201e0FDF1Bd54799712;
            pid = _pid;
            earnedAddress = 0x6FC4563460d5f45932C473334d5c1C5B4aEA0E01;

            earnedToNATIVEPath = [earnedAddress, WWDOGE_ADDRESS, retroAddress];
            if (WWDOGE_ADDRESS == earnedAddress) {
                earnedToNATIVEPath = [WWDOGE_ADDRESS, retroAddress];
            }

            earnedToToken0Path = [earnedAddress, WWDOGE_ADDRESS, token0Address];
            if (WWDOGE_ADDRESS == token0Address) {
                earnedToToken0Path = [earnedAddress, WWDOGE_ADDRESS];
            }

            earnedToToken1Path = [earnedAddress, WWDOGE_ADDRESS, token1Address];
            if (WWDOGE_ADDRESS == token1Address) {
                earnedToToken1Path = [earnedAddress, WWDOGE_ADDRESS];
            }

            token0ToEarnedPath = [token0Address, WWDOGE_ADDRESS, earnedAddress];
            if (WWDOGE_ADDRESS == token0Address) {
                token0ToEarnedPath = [WWDOGE_ADDRESS, earnedAddress];
            }

            token1ToEarnedPath = [token1Address, WWDOGE_ADDRESS, earnedAddress];
            if (WWDOGE_ADDRESS == token1Address) {
                token1ToEarnedPath = [WWDOGE_ADDRESS, earnedAddress];
            }

            earnedToWantPath = [earnedAddress, WWDOGE_ADDRESS, wantAddress];
            if (WWDOGE_ADDRESS == wantAddress) {
                earnedToWantPath = [earnedAddress, wantAddress];
            }

            earnedToWWDOGEPath = [earnedAddress, WWDOGE_ADDRESS];
            wwdogeToNativePath = [WWDOGE_ADDRESS, retroAddress];
        }
    ...
    function buyBack(uint256 earnedAmt) internal returns (uint256) {
        ...
        if (UNI_ROUTER_ADDRESS != buybackRouterAddress) {
            ...
            if (earnedAddress != WWDOGE_ADDRESS) {
                // First convert earn to WWDOGE
                IERC20(earnedAddress).safeIncreaseAllowance(
                    UNI_ROUTER_ADDRESS,
                    buyBackAmt
                );
            ...
            // convert all WWDOGE to Native and burn them
            uint256 WWDOGEAmt = IERC20(WWDOGE_ADDRESS).balanceOf(address(this));
            if (WWDOGEAmt > 0) {
                IERC20(WWDOGE_ADDRESS).safeIncreaseAllowance(
    ...
    function pause() external onlyAllowGov {
        _pause();
        IXswapFarm(FARM_CONTRACT_ADDRESS).emergencyWithdraw(pid);
    }
    ...
    function changeRetroAddress(address newRetroAddress) external onlyAllowGov {
        ...
        earnedToNATIVEPath = [earnedAddress, WWDOGE_ADDRESS, newRetroAddress];
        if (WWDOGE_ADDRESS == earnedAddress) {
            earnedToNATIVEPath = [WWDOGE_ADDRESS, newRetroAddress];
        }
        wwdogeToNativePath = [WWDOGE_ADDRESS, newRetroAddress];
    }

    function changeBuyBackAddress(address newBuyBackAddress)
        external
        onlyAllowGov
    {
        require(newBuyBackAddress != address(0), "BuyBackAddress cannot be 0");
        require(
            newBuyBackAddress != retroAddress,
            "BuyBackAddress cannot be same"
        );
        BUY_BACK_ADDRESS = newBuyBackAddress;

        earnedToNATIVEPath = [earnedAddress, WWDOGE_ADDRESS, newBuyBackAddress];
        if (WWDOGE_ADDRESS == earnedAddress) {
            earnedToNATIVEPath = [WWDOGE_ADDRESS, newBuyBackAddress];
        }
        wwdogeToNativePath = [WWDOGE_ADDRESS, newBuyBackAddress];
    }

    function workerCompound() external view returns (uint256) {
        uint256 BalanceTokens = IERC20(earnedAddress).balanceOf(address(this));
        uint256 Comp = IXswapFarm(FARM_CONTRACT_ADDRESS).pendingYode(
            pid,
            address(this)
        );
